...
function checkAutoBackup() {
    ...
    if (!githubToken || !dataChanged) return;
    
    const now = Date.now();
    const oneMinute = 60 * 1000;
    
    if (!lastBackup || (now - parseInt(lastBackup)) > oneMinute) {
    ...
    if (gistId) {
        ...
        fetch('https://api.github.com/gists', {
            ...
            if (data) {
                ...
                localStorage.setItem('lastAutoBackup', Date.now());
                localStorage.removeItem('dataChanged');
                console.log('Auto backup created:', data.html_url);
            }
        }).catch(err => console.log('Auto backup failed'));
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    initializeApp();
    // Start backup checking after app is initialized
